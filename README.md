...
자바 **162파일 / 약 17,000줄** · 테스트 **534개** · 설계 결정 **82건**

수치는 전부 실제 세이브 3개(공식 800 + 스크림 407 = **1,207경기**, 챔피언 40종)에서 측정했다.

---

## 이 저장소에서 볼 것 — 개발 워크플로우

혼자 만들었지만 **AI 에이전트와 같이** 만들었다. 그래서 이 저장소의 절반은 코드가 아니라
**무엇을 왜 그렇게 정했는지, 그리고 에이전트에게 무엇을 넘기고 무엇을 안 넘기는지**에 대한 기록이다.

그 규칙들은 앉아서 설계한 것이 아니라 **깨지면서 고친 것**이다. 아래 다섯 개가 그 결과다.
...
파일 하나에 결정 하나. 파일명이 곧 제목이다.

**모든 결정에 「뒤집힐 조건」을 같이 적는다.** 근거가 사라지면 결정도 다시 봐야 하기 때문이다.
...
낡은 문서는 없느니만 못하다. 그래서 세션 시작에 읽는 두 문서에 **한도**를 걸었다.
...
넘치면 내용을 `decisions/` 로 내려보낸다. 둘을 합친 6KB가 **세션 시작의 정액 예산**이다.
...
| | |
|---|---|
| **안 넘긴다** | 설계 판단 · 구현 · 문서 갱신 · 커밋 · 최종 결정 |
| **넘긴다** | 파일을 많이 읽어야 하지만 **판단은 필요 없는** 일 (리뷰 · 스키마 점검 · 삼켜진 오류 찾기 · 죽은 코드) |
...
### 4. 테스트는 통과가 아니라 **변조로** 검증한다

> **이 단언이 검증 대상 말고 다른 경로로도 만족되는가?**

통과하는 테스트는 그 자체로는 아무것도 증명하지 않는다. 그래서 **막으려는 버그를 실제로 코드에 넣고,
실패하는지 본다.** 통과하면 그건 테스트가 아니다.
