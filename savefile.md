...
`Count=212` 가 학습 표본 수인지. 자세한 판단은 `decision.md` D32.
...
가변 규칙이 아니다. 다만 **챔피언 풀이 작던 커리어 초반 데이터에는 팀당 밴이 2개**로 남아 있다. slot 638064(등장 챔피언 31종,
...
과거 2밴 경기를 읽을 때 인덱스 3을 건드리면 깨진다. 그리고 밴률을 커리어 초반 구간과 이후 구간에
...
- 밴이 없다
- **팀 인원이 4명이 아닐 수 있다.** slot 638064 는 151건 중 2명(2v2) 17건, 3명(3v3) 21건, 4명 113건.
  나머지 두 슬롯은 전부 4명. **4v4 가 아닌 스크림은 카운터·시너지 집계에서 제외한다** —
  같은 조건이 아니라 승률을 섞으면 안 된다
...
**2차 밴이 픽 4개 공개 뒤에 온다** — 밴픽 시뮬에서 이 두 밴은 성격이 다르다 (decision.md D26).
