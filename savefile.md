...
**밴픽·챔피언·개인스탯이 전혀 저장되지 않는다.** 스케줄에는 스코어가 남고,
**킬도 남는다** — `MatchScheduleParser` 로 확인했다(이벤트 5건 중 치러진 것 전부 킬>0).
기사가 이벤트전을 다룰 때 쓸 숫자가 아주 없지는 않다.
...
**조인 키는 `(Season, Day, {두 팀})` 이다 — 팀 순서를 가리지 않는다.** 실측으로 이 키는
190건 전부 고유하고 경기 294건이 하나도 빠짐없이 붙는다. `ScheduleID` 단독으로는 122건이
모호해진다(190건이 114개 값에 겹친다).

**팀 순서를 지키면 안 되는 이유 — 세트마다 진영이 바뀐다.** 294세트 중 **122세트**가
매치 기준(`MatchSchedule.BlueTeam`)과 진영이 반대다. 순서 있는 키로 조인하면 그 122건이
통째로 빠진다.

**검산이 딸려 온다.** 세트 승수의 합 = `MatchSchedule.BlueScore/RedScore`,
세트 킬의 합 = `BlueKill/RedKill` 이 **109/109 매치에서 정확히 일치**한다.
조인과 진영 처리가 맞는지 이 두 등식으로 확인할 수 있다.
