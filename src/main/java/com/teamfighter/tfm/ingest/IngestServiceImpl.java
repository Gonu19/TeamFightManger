...
/**
 * {@link IngestService} 구현.
 *
 * <p>이 클래스의 규칙은 하나로 요약된다 — <b>파서는 관대하고 적재는 엄격하다.</b>
...
public class IngestServiceImpl implements IngestService {
    ...
    public IngestServiceImpl(SaveSlotRepository slots,
    ...
    public IngestResult ingest(Path saveFile) {
        ...
        SaveSlot slot = slots.findBySlotKey(slotKey)
                .orElseGet(() -> slots.save(new SaveSlot(slotKey, null)));

        if (runs.existsBySlotIdAndFileHash(slot.getSlotId(), hash)) {
            log.debug("이미 적재된 내용이다: {} ({})", slotKey, hash.substring(0, 12));
            return IngestResult.duplicate(slot.getSlotId());
        }

        IngestRun run = runs.save(new IngestRun(slot.getSlotId(), hash, size, PARSER_VERSION));
        try {
            IngestResult result = load(slot, saveFile);
            run.succeed(result.newMatches(), result.newScrims(), OffsetDateTime.now());
            slot.markIngested(OffsetDateTime.now());
            return result;
        } catch (RuntimeException e) {
            // 실패를 기록에 남긴다. 조용히 사라지면 다음 적재가 같은 곳에서 다시 넘어진다.
            run.fail(e.toString(), OffsetDateTime.now());
            throw e;
        }
    }
    ...
    private IngestResult load(SaveSlot slot, Path saveFile) {
        ParsedSave save;
        try {
        ...
        return new IngestResult(slot.getSlotId(), official.saved, scrim.saved,
    ...
    private PatchAssigner buildAssigner(SaveSlot slot) {
        ...
        for (Patch p : slotPatches) {
            for (ChampionPatchEvent e : patchEvents.findByIdPatchId(p.getPatchId())) {
                changeSeq.computeIfAbsent(e.getId().getChampionId(), k -> new ArrayList<>()).add(p.getSeq());
            }
        }
    ...
    private Counts saveGames(SaveSlot slot, List<ParsedGame> games,
        ...
        for (ParsedGame g : games) {
            ...
            MatchRecord match = new MatchRecord(slot.getSlotId(), MatchType.OFFICIAL, g.id(),
